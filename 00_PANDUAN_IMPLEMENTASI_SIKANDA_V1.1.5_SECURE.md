# Panduan Implementasi SIKANDA V1.1.5 Secure

Panduan ini adalah sumber utama untuk upgrade dari **SIKANDA V1.1.4 Secure** ke **V1.1.5 Secure**. Gunakan seluruh file dari paket yang sama. Jangan mencampurkan frontend, `Code.gs`, atau panduan dari rilis berbeda.

## 1. Ringkasan keputusan implementasi

- Upgrade V1.1.4 → V1.1.5 **tidak membutuhkan SQL baru**. Struktur `pegawai`, `assets_vehicle`, dan `assets_equipment` yang sudah dipasang pada V1.1.3 telah memuat field yang diperlukan.
- Backend `apps-script/Code.gs` wajib diganti seluruhnya dan dibuatkan **New version** deployment.
- Seluruh source frontend V1.1.5 wajib di-upload/replace ke Google AI Studio dan GitHub. Mengubah Secrets saja tidak mengubah UI.
...
2. Ganti seluruh isi `Code.gs` dengan `apps-script/Code.gs` V1.1.5.
3. Buat New version deployment Apps Script dan salin URL `/exec`.
4. Perbarui `VITE_APPS_SCRIPT_URL` hanya bila URL berubah.
5. Upload/replace seluruh source V1.1.5 di Google AI Studio, klik **Apply changes**, uji Preview, lalu **Publish**.
6. Push source yang sama ke GitHub `main` dan tunggu GitHub Actions selesai hijau.
7. Lakukan validasi pada Bagian 9.

## 2. Isi revisi V1.1.5

### Akurasi lanjutan V1.1.5

- Tanya SIKANDA menghitung ulang tahun, ASN/PPPK, agenda, dan ringkasan dari pegawai aktif pada request berjalan; parser tanggal mendukung bulan Indonesia dan Inggris.
- Pertanyaan ulang tahun membedakan rentang hari ini, tujuh hari mendatang, dan bulan berjalan serta dapat memeriksa pegawai yang disebut namanya.
- Klik setiap notifikasi membuka profil tepat berdasarkan NIP; panel tidak terpotong pada viewport mobile.
- Kode Barang dan Nomor Polisi tidak lagi memakai fallback silang. Peta menampilkan Pengguna/Penanggung Jawab yang dicocokkan secara aman ke nama baku Database Pegawai.
- Basemap mobile dibuka lewat tap/click dan peta menghitung ulang ukuran saat orientasi/viewport berubah.
- Checkbox seleksi massal Kendaraan dan Alat & Mesin dihapus dari tampilan.
- Data Cleansing memakai Database Pegawai sebagai satu-satunya sumber saran nama dan mengabaikan kandidat ambigu.
...
Untuk upgrade dari V1.1.4, **jangan menjalankan SQL baru** karena rilis ini tidak mengubah skema. File SQL `001`, `002`, dan `003` tetap disertakan untuk instalasi bersih/riwayat migrasi.
...
2. Buka `Code.gs`.
3. Pilih seluruh isi lama lalu ganti dengan seluruh isi `apps-script/Code.gs` V1.1.5.
4. Klik Save.
...
Tidak ada Script Property baru pada V1.1.5.
...
```json
{"ok":true,"service":"SIKANDA","version":"1.1.5-secure"}
```

## 6. Google AI Studio

1. Buat checkpoint project V1.1.4.
2. Upload/import ZIP V1.1.5 atau replace seluruh file source dengan isi ZIP.
...
Push seluruh source V1.1.5, kecuali `node_modules`, `dist`, `.test-dist`, dan file secret. Repository Secrets minimal:
...
1. URL Apps Script menampilkan `1.1.5-secure`.
...
1. Kembalikan checkpoint/source V1.1.4 pada AI Studio atau commit GitHub sebelumnya.
2. Arahkan `VITE_APPS_SCRIPT_URL` ke deployment backend V1.1.4 bila diperlukan.
3. Tidak ada rollback SQL karena V1.1.5 tidak mengubah skema.
4. Jangan menghapus deployment lama sampai seluruh validasi V1.1.5 selesai.

## 11. Indikator selesai

Upgrade dinyatakan selesai setelah backend melaporkan `1.1.5-secure`, source V1.1.5 sudah dipublish pada AI Studio dan GitHub, seluruh build/test hijau, Dashboard mengikuti data aktif terbaru, foto/GPS aset berfungsi, cetak rapi, notifikasi ulang tahun muncul, dan Tanya SIKANDA lulus pengujian faktual.
