# PANDUAN IMPLEMENTASI SIKANDA V1.1.10 SECURE

Paket utama: `SIKANDA_v1.1.10_SECURE_AI_STUDIO_FINAL.zip`  
Target backend: `1.1.10-secure`  
...
| `assets_equipment` | 40 | 1 | 39 |

Artinya database tidak menyatakan seluruh aset BAIK. V1.1.9 menampilkan kondisi kosong sebagai BAIK karena fallback frontend. Risiko yang lebih besar: saat pengguna mengedit field lain, payload lama juga dapat mengirim BAIK sehingga fakta kosong berubah tanpa verifikasi.

V1.1.10 memperbaiki sumber masalah tanpa mengarang data:
...
- perbaikan 172 record dilakukan individual setelah pemeriksaan fisik.
...
- Card kondisi dibentuk dari data nyata, termasuk bucket **BELUM DIISI**.
...
- Apps Script melaporkan `1.1.10-secure`.
...
- `apps-script/Code.gs`: backend V1.1.10 lengkap.
- `dist/`: build produksi tervalidasi.
- `tests/`: 12 suite pengujian, termasuk regresi V1.1.10.
- `supabase/001...005`: migrasi historis; **tidak perlu dijalankan ulang**.
- `.env.example`: daftar variable frontend tanpa nilai rahasia.
- `RELEASE_NOTES.md` dan `VERIFICATION_REPORT_V1.1.10.md`.

## 4. Persiapan dan backup

Sebelum upgrade:

1. Unduh/commit source V1.1.9 yang sedang live.
2. Catat commit GitHub Pages yang aktif.
3. Pada Apps Script, catat version deployment V1.1.9.
...
V1.1.10 tidak memerlukan perubahan skema. Backup tetap wajib karena UAT melakukan transaksi nyata.

## 5. Instalasi Google AI Studio

1. Gunakan ZIP `SIKANDA_v1.1.10_SECURE_AI_STUDIO_FINAL.zip`.
2. Import ke project SIKANDA.
3. Pastikan file penting terlihat:
   - `package.json` versi `1.1.10`;
   - `src/lib/assetCondition.ts`;
   - `tests/revision-v1110.test.ts`;
   - `apps-script/Code.gs`.
...
3. Ganti seluruh isinya dengan `apps-script/Code.gs` dari paket V1.1.10.
4. Simpan.
...
```json
{"ok":true,"service":"SIKANDA","version":"1.1.10-secure"}
```

10. Jika URL berubah, perbarui `VITE_APPS_SCRIPT_URL` dan build ulang frontend.

Tidak ada Script Property baru. Jangan menjalankan ulang trigger atau migrasi foto hanya untuk V1.1.10.

## 7. Deploy GitHub Pages

1. Salin seluruh source V1.1.10 ke repository SIKANDA.
...
7. Pastikan favicon SIKANDA muncul dan tidak ada bundle V1.1.9 tersisa dari cache.

## 8. SQL: tidak ada migrasi baru

V1.1.10 adalah koreksi aplikasi dan backend. Jangan menjalankan migrasi 001–005 ulang.
...
Pada konfigurasi saat ini, hasil yang ditunjukkan capture hanya `service_role`. Ini sesuai arsitektur frontend → Apps Script → Supabase. RLS dan grant perlu dinilai bersama; grant service-role saja bukan satu-satunya bukti keamanan.
...
2. Pastikan card menunjukkan total 139 dan bucket BELUM DIISI 133 selama database belum diperbaiki.
3. Buka Alat & Mesin.
4. Pastikan total 40 dan BELUM DIISI 39.
5. Klik card BELUM DIISI; jumlah tabel harus sama dengan angka card.
...
Source V1.1.10 adalah **production candidate** dengan kontrol berikut:
...
- `/exec` menunjukkan `1.1.10-secure`;
- GitHub Pages memuat bundle V1.1.10;
...
1. Pada GitHub Pages, redeploy commit V1.1.9 yang sebelumnya aktif.
2. Pada Apps Script, edit deployment dan pilih version V1.1.9 yang dicatat saat backup.
3. Lakukan hard refresh.
4. Tidak ada rollback SQL karena V1.1.10 tidak mengubah skema.
...
- [ ] Card kondisi menampilkan BAIK dan BELUM DIISI secara faktual.
...
Setelah seluruh checklist ditandatangani, V1.1.10 dapat dipromosikan dari production candidate menjadi release production pada environment SIKANDA.
