...
async function run() {
  ...
  const challenge = await req({ action: 'captcha_challenge', purpose: 'login', clientKey: 'curl-test' });
  if (!challenge.ok) return console.log('Challenge failed', challenge);

  const proof = {
    challengeId: challenge.challengeId,
    position: challenge.x,
    elapsedMs: 2000,
    track: [10, 20, 30, challenge.x]
  };

  const login = await req({ action: 'auth_login', nip: '111111111111111111', password: 'IceTo152041gmail', captcha: proof, clientKey: 'curl-test' });
