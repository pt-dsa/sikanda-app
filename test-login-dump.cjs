async function run() {
  ...
  const challenge = await req({ action: 'captcha_challenge', purpose: 'login', clientKey: 'curl-test-dump' });
  ...
  const login = await req({ action: 'auth_login', nip: '111111111111111111', password: 'IceTo152041gmail', captcha: proof, clientKey: 'curl-test-dump' });
